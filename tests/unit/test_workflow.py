...
def test_spin_projections_reenabled_through_problem_sets():
    ...
    def get_particle_name(prop_map: GraphEdgePropertyMap) -> str:
        pid = prop_map[EdgeQuantumNumbers.pid]
        assert pid is not None
        return particle_db.find(int(pid)).name
...
def test_generate_qn_transitions():
    ...
    collapsed_dot = asdot(reaction, collapse="topology")
    assert "0⁺(0⁺⁺)" in collapsed_dot
    collapsed_mermaid = asmermaid(reaction, collapse="topology")
